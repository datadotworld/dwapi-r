...
dw_test_that("deleteFilesAndSyncSources making the correct HTTR request", {
  response <- with_mock(
    `httr::DELETE` = function(url, header, user_agent)  {
      expect_equal(
        url,
        paste("https://api.data.world/v0/datasets/",
          "ownerid/datasetid/files?name=file1.csv&name=file2.csv",
          sep = "")
      )
...
dw_test_that("deleteFileAndSyncSources making the correct HTTR request", {
  response <- with_mock(
    `httr::DELETE` = function(url, header, user_agent)  {
      expect_equal(url,
